...
def find_message_with_text(
    responses: List[BotResponse], text: str
...
def find_message_by_id_in_list(
    messages: List[BotResponse], message_id: int
) -> Optional[BotResponse]:
...
async def test_ping(teletest_client, ping_bot):
    ...
    responses = await teletest_client.send_message(
...
async def test_buttons_and_press(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"

    # Send /buttons command
    buttons_responses = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/buttons", timeout_sec=5
        )
    )
    ...
    press_a_responses = await teletest_client.press_button(
    ...
    assert not any(
        r.message_text
        for r in press_a_responses
        if r.response_type == ResponseType.MESSAGE
    ), (
    ...
    await asyncio.sleep(1)  # Give a moment for the edit to propagate if necessary
    updates_a = await teletest_client.get_messages(bot_username=bot_username, limit=5)

    edited_message_a = find_message_by_id_in_list(
        updates_a.messages, original_message_id_for_a
    )
    assert edited_message_a, (
        f"Original message ID {original_message_id_for_a} not found in updates after pressing A"
    )
    ...
    button_messages_again_list = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/buttons", timeout_sec=5
        )
    )
    ...
    assert choose_message_for_b, (
        "Could not find 'Choose:' message when re-sending /buttons for button B test"
    )

    press_b_responses = await teletest_client.press_button(
    ...
    new_message_for_b = find_message_with_text(
        press_b_responses, "Additionally, I sent a new message because you chose B."
    )
    assert new_message_for_b, (
        "Did not find new message 'Additionally, I sent a new message because you chose B.' after pressing button B"
    )
    assert new_message_for_b.response_type == ResponseType.MESSAGE
    assert isinstance(new_message_for_b.message_id, int)


async def test_get_messages(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"
    ...
    await teletest_client.get_messages(bot_username=bot_username, limit=100)

    # Send a message to trigger a response from the bot
    ping_data_list = await teletest_client.send_message(
        SendMessageRequest(bot_username=bot_username, message_text="/ping")
    )
    ...
    assert pong_message, (
        "No 'pong' message found in send-message response for get_messages test"
    )
    ...
    # Now get messages
    resp = await teletest_client.get_messages(bot_username=bot_username, limit=1)
    msgs = resp.messages
...
async def test_send_message_timeout(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"
    ...
    data_list = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/nonexistentcommand", timeout_sec=1
        )
    )
...
async def test_get_updates(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"

    # Send a message to ensure there's something to fetch
    send_data_list = await teletest_client.send_message(
        SendMessageRequest(bot_username=bot_username, message_text="/ping")
    )

    # Ensure the bot responded to /ping
    assert any(m.message_text == "pong" for m in send_data_list), (
        "Bot did not respond with 'pong' to /ping"
    )

    # Now get updates
    updates_resp = await teletest_client.get_messages(
        bot_username=bot_username, limit=5
    )
    ...
    found_pong = any(
        msg.response_type == ResponseType.MESSAGE and msg.message_text == "pong"
        for msg in msgs
    )
    ...
    if len(msgs) > 1:
        ...
        assert msgs[0].message_id < msgs[-1].message_id, (
            "Messages do not appear to be in chronological order"
        )


async def test_edit_message_command(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"

    # Send /edit_test command
    send_responses = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/edit_test", timeout_sec=5
        )
    )

    # The bot first sends "Original message, I will edit this."
    original_sent_msg = find_message_with_text(
        send_responses, "Original message, I will edit this."
    )
    assert original_sent_msg, (
        f"Initial message from /edit_test not found in {send_responses}"
    )
    ...
    assert edited_message, (
        f"Message ID {original_message_id} not found in updates for /edit_test: {updates_data}"
    )
    assert edited_message.message_text == "Edited message!"


async def test_alert_callback(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"
    ...
    send_responses = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/alert_test", timeout_sec=5
        )
    )

    alert_msg_with_button = find_message_with_text(
        send_responses, "Press the button to see an alert."
    )
    assert alert_msg_with_button, (
        f"Message from /alert_test with button not found in {send_responses}"
    )
    ...
    press_responses = await teletest_client.press_button(
        PressButtonRequest(
            bot_username=bot_username, button_text="Show Alert", timeout_sec=5
        )
    )
...
async def test_new_message_from_callback(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"
    ...
    send_responses = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/new_message_test", timeout_sec=5
        )
    )

    initial_msg = find_message_with_text(
        send_responses, "Press the button and I will send a new message."
    )
    assert initial_msg, (
        f"Message from /new_message_test with button not found in {send_responses}"
    )
    ...
    press_responses = await teletest_client.press_button(
        PressButtonRequest(
            bot_username=bot_username, button_text="Send New Msg", timeout_sec=5
        )
    )

    brand_new_message = find_message_with_text(
        press_responses, "This is a brand new message triggered by the button."
    )
    assert brand_new_message, (
        f"Did not find the new message triggered by 'Send New Msg' button in {press_responses}"
    )
    assert brand_new_message.response_type == ResponseType.MESSAGE
    assert isinstance(brand_new_message.message_id, int)


async def test_ack_callback(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"
    ...
    send_responses = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/ack_test", timeout_sec=5
        )
    )

    initial_msg = find_message_with_text(
        send_responses, "Press the button for a simple acknowledgement."
    )
    assert initial_msg, (
        f"Message from /ack_test with button not found in {send_responses}"
    )
    ...
    press_responses = await teletest_client.press_button(
        PressButtonRequest(
            bot_username=bot_username, button_text="Just Ack", timeout_sec=5
        )
    )
...
async def test_delay_test(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"
    ...
    send_responses = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/delay_test", timeout_sec=5
        )
    )

    assert len(send_responses) == 2, (
        f"Expected 2 messages from /delay_test, got: {send_responses}"
    )
    assert send_responses[0].message_text == "Waiting for 3 seconds..."
    assert send_responses[1].message_text == "Done waiting!"


async def test_reply_keyboard(teletest_client, ping_bot):
    bot_username = os.getenv("TELEGRAM_TEST_BOT_USERNAME")
    assert bot_username, "TELEGRAM_TEST_BOT_USERNAME environment variable not set"

    # Send command to show reply keyboard
    show_responses = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/reply_kb", timeout_sec=5
        )
    )
    ...
    press_responses = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="Option 1", timeout_sec=5
        )
    )
    selected = find_message_with_text(press_responses, "You chose option 1")
    assert selected, (
        f"Response to reply keyboard selection not found in {press_responses}"
    )
    assert selected.reply_keyboard is False

    # Remove keyboard to not interfere with other tests
    remove_responses = await teletest_client.send_message(
        SendMessageRequest(
            bot_username=bot_username, message_text="/remove_kb", timeout_sec=5
        )
    )
